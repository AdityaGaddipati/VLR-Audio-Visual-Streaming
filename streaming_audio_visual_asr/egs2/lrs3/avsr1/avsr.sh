...
# Multimodal Related - uncomment if unimodal
audio_visual=false
mouth_roi=false
vis_step=1
...
Options:
    ...
    # Multimodal Related
    --audio_visual   # whether to use multimodal data/models default=false
    --vis_step # number of frames of audio frames a single visual frame will be spanning
    --mouth roi # whether to use mouth roi cropped videos
EOF
)
...
if ! "${skip_data_prep}"; then
    if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
        ...
        local/data.sh --audio_visual ${audio_visual} --mouth_roi ${mouth_roi} \ ${local_data_opts}
    fi
        ...
        if [ -n "${speed_perturb_factors}" ]; then
           ...
           for factor in ${speed_perturb_factors}; do
               if [[ $(bc <<<"${factor} != 1.0") == 1 ]]; then
                   ...
                   if "${audio_visual}"; then
                      cp "data/${train_set}/vision" "data/${train_set}_sp${factor}/vision"
                   fi
               else
           ...
           if "${audio_visual}"; then
                for factor in ${speed_perturb_factors}; do
                    if [[ $(bc <<<"${factor} != 1.0") == 1 ]]; then
                        ...
                        # If speed factor is 1, same as the original
                        cp "data/${train_set}/vision" "data/${train_set}_sp/vision"
                    fi
                done
           fi
    ...
    if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
        ...
        if "${audio_visual}"; then
            log "[Multimodal] Stage 3: Vision extract: data/ -> ${data_feats}"
            for dset in "${train_set}" "${valid_set}" ${test_sets}; do
                mkdir -p "${data_feats}/${dset}"
                cp data/"${dset}"/vision "${data_feats}/${dset}/"
            done
        fi
    fi
...
if ! "${skip_train}"; then
    ...
    if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ]; then
        ...
        if "${audio_visual}"; then
            _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/vision,vision,vision "
            _opts+="--valid_data_path_and_name_and_type ${_asr_valid_dir}/vision,vision,vision "
        fi
