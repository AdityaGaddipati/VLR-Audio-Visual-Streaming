...
class Utils:
    ...
    def get_parser():
        ...
        parser.add_argument("--audio_visual", type=bool, default = False, help="True for Multimodal Option Using Vision Features")
        parser.add_argument("--mouth_roi", type=bool, default = False, help="True for using Mouth ROI Vision Features")
        return parser
...
class DatasetUtils:
    ...
    @staticmethod
    def perform_data_prep(speaker_folders: list, dataset: str, audio_visual: bool = False, mouth_roi: bool = False) -> None:
        ...
        if audio_visual:
            ...
            vision_file = os.path.join("data", dataset, "vision")
...
def main():
    ...
    audio_visual = args.audio_visual

    logging.info(f"Performing Data Preparation for TEST")
    DatasetUtils.perform_data_prep(test_folders, "test", audio_visual = audio_visual, mouth_roi = args.mouth_roi)

    logging.info(f"Performing Data Preparation for TRAIN")
    DatasetUtils.perform_data_prep(train_folders, "train", audio_visual = audio_visual, mouth_roi = args.mouth_roi)

    logging.info(f"Performing Data Preparation for DEV")
    DatasetUtils.perform_data_prep(dev_folders, "dev", audio_visual = audio_visual, mouth_roi = args.mouth_roi)
