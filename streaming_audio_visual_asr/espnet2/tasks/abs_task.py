...
class AbsTask(ABC):
    ...
    def get_parser(cls) -> config_argparse.ArgumentParser:
        ...
        group = parser.add_argument_group("Vision related")
        
        group.add_argument(
            f"--audio_visual",
            type=bool,
            default=False,
            help="Option to use Audio Visual Multimodal Speech Recognition",
        )
        group.add_argument(
            f"--vis_step",
            type=int_or_none,
            default=1,
            help="Number of audio frames that a single vision video capture should span",
        )
    ...
    def main_worker(cls, args: argparse.Namespace):
        ...
        if args.dry_run:
            ...
            # 7-2. Transfer Loader Param Rates to Model for audio-visual alignment
            if args.audio_visual:
                audio_rate, video_rate = train_iter_factory.dataset.get_audio_video_rates()
                model.set_audio_video_rates(audio_rate, video_rate)
    ...
    def build_iter_factory(
        ...
        if args.iterator_type == "sequence":
            # For Multimodal AVSR - sequence iter factory is built
    ...
    def build_sequence_iter_factory(
        ...
        assert check_argument_types()
        # IteratorOptions(preprocess_fn=<espnet2.train.preprocessor.CommonPreprocessor object at 0x7fde248fc820>, 
        # collate_fn=<class 'espnet2.train.collate_fn.CommonCollateFn'>(float_pad_value=0.0, int_pad_value=0.0), 
        # data_path_and_name_and_type=[('dump/raw/train_sp/wav.scp', 'speech', 'sound'), 
        #                              ('dump/raw/train_sp/text', 'text', 'text')], 
        # shape_files=['exp/asr_stats_raw_en_bpe5000_sp/train/speech_shape', 'exp/asr_stats_raw_en_bpe5000_sp/train/text_shape.bpe'], 
        # batch_size=20, batch_bins=16000000, batch_type='numel', 
        # max_cache_size=0.0, max_cache_fd=32, distributed=False, num_batches=None, num_iters_per_epoch=None, train=True)
        ...
        dataset = ESPnetDataset(
            ...
            vis_step=iter_options.vis_step,
        )
        cls.check_task_requirements(
