...
from espnet2.fileio.audio_visual_dataset import VisionDataset, VisionFileReader
...
class AdapterForSoundScpReader(collections.abc.Mapping):
    ...
    def set_rate(self):
        ...
        if isinstance(retval, tuple):
            ...
            # TODO: Fill in for normal ark case
            self.rate = None
...
def vision_loader(path, audio_rate = 24, vis_step = 10, float_dtype=None):
    # audio_rate: audio frame rate
    # vis_step: expected span of auudio frames per captured video image

    loader = VisionFileReader(path, normalize=True)
    return VisionDataset(loader, audio_rate, vis_step, float_dtype)
...
DATA_TYPES = {
    ...
    "vision": dict(
        func=vision_loader,
        kwargs=["audio_rate", "vis_step", "float_dtype"],
...
class ESPnetDataset(AbsDataset):
    ...
    def __init__(
        ...
        for path, name, _type in path_name_type_list:
            if(name == "speech"): 
                ...
                self.rate = loader.get_rate()
            elif(name == "vision"):
                loader = self._build_loader(path, _type)
                self.vid_rate = loader.get_vid_rate()
                self.vid_sample_rate = loader.get_sample_rate()
            else:
                continue
    ...
    def get_audio_video_rates(self):
        return self.rate, self.vid_sample_rate
            ...
            if re.match(key, loader_type):
                ...
                for key2 in dic["kwargs"]:
                    if key2 == "loader_type":
                        ...
                        kwargs["vis_step"] = self.vis_step
                    elif key2 == "audio_rate":
                        kwargs["audio_rate"] = self.rate
                    else:
